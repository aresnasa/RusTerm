...
130. ![](image_11.png)登录 jumpserver 需要用如下逻辑：
  ...
  0.15 继续检查飞书搜索逻辑，没有正确的搜索智小安，导致无法发送消息，继续优化代码
  0.16 检查复制会话的逻辑（可以不用复制 jumpserver 这种交互式的前几步，使用正则检查节点是否已经正确的登录了即可，可以减少重复输入），需要优化这块的逻辑
  0.17 优化一下恢复逻辑，这里检查 jumpserver 的登录状态，需要复用第一台的 otp，这里其他回放可以等待，等第一个 otp 认证成功（我会提供 otp 二次密码），然后再使用已有的 ssh 密钥来回放，这样就能少输入 otp 了，改造相关逻辑
  0.18 [@Image](zed:///agent/pasted-image?name=Image) 恢复回放的时候要除了第一个需要等 OTP 的终端外，其他终端都要等待，直到 OTP 被第一个终端处理完获取到相关的 ssh 信息后才能继续登录，改造相关逻辑。
  ...
  0.26 [@Image](zed:///agent/pasted-image?name=Image) 会话的宽度要动态的和主机名匹配，动态的显示
  0.27 这里可以高亮 hostname，不要用暗色，方便用户确认登录的机器情况
  0.28
