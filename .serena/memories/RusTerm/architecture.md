...
## v0.25 — tab hostname row below name (taller/narrower tabs) (2026-08-08)

Replaces the v0.24 side-by-side layout (user 0.25: "登录的会话主机名放单 jumpserver 的下方，然后增加会话的高度减小长度，让会话更高"). `components/tab_bar.rs`: the two v0.24 wrapped spans (name 320px + node suffix 400px, rendered `" · {node}"`) are replaced by a vertical column div (`display: flex; flex-direction: column; justify-content: center; gap: 1px; max-width: 220px; min-width: 0`) holding name span (single-line nowrap ellipsis, `line-height: 1.2`) above node span (`font-size: 11px`, muted, single-line ellipsis, rendered as bare `{node}` — " · " prefix dropped since it's its own row now, still gated by the OSC-7-differs-from-host check). Effect: tab gains height (bar stays `min-height: 36px`, auto-grows) and loses width vs v0.24; number/dot/badge/close remain row siblings centered via outer `align-items: center`; outer tab div keeps `white-space: nowrap` + `data-rusterm-tab-id` so Task 22 manual drag hit-testing (elementFromPoint().closest) still resolves from the inner column. Supersedes v0.24 widths. Pure style change, no new tests; rusterm-ui lib 866 green, `cargo fmt` clean.

## v0.26 — tab width auto-fits hostname (2026-08-08)

Supersedes v0.25's `max-width: 220px` ellipsis cap (user: "会话的宽度要动态的和主机名匹配，动态的显示" — screenshot showed the node FQDN truncated to "…"). `components/tab_bar.rs`: the vertical column div drops `max-width: 220px` / `min-width: 0` and gains `flex-shrink: 0` so crowded sibling tabs can't re-crush it back into an ellipsis look (flex default `flex-shrink: 1` would otherwise shrink the column once max-width was gone — the key gotcha). Both the name span and the node span drop `overflow: hidden` + `text-overflow: ellipsis`, keeping `white-space: nowrap`, so each row renders full text and the tab width = intrinsic width of the longer row (usually the full node hostname, e.g. `lg-cmc-q-prod-k8s-master-0001.host.lg.shzhisuan.com`). No cap at all now — intentionally: the user explicitly rejected truncation. Vertical stacking (v0.25), node-row gating (OSC-7 differs from connection host), number/dot/badge/close row siblings, outer `white-space: nowrap` and `data-rusterm-tab-id` (Task 22 drag hit-testing) all unchanged. Pure style change, no new tests; rusterm-ui lib 866 green, `cargo fmt` clean.
