...
fn translate<'a>(key: &str, lang: Language) -> Option<&'a str> {
    let (en, zh): (&'a str, &'a str) = match key {
        ...
        "chat.need_api_key" => (
            "This agent has no API key — open ⚙ and paste one (kept in memory only, never written to disk).",
            "该智能体尚未配置 API Key——请点击 ⚙ 填写（仅保存在内存中，不会写入磁盘）。",
        ),
        "chat.need_model" => (
            "This agent has no model configured — open ⚙ and set one, or apply an official preset.",
            "该智能体尚未配置模型——请点击 ⚙ 设置，或应用一个官方推荐配置。",
        ),
        "chat.request_failed" => ("Request failed", "请求失败"),
        "chat.request_in_flight" => ("A request is already in flight…", "上一条请求仍在处理中…"),
        ...
        "chat.api_key_saved" => ("API key saved (in memory)", "API Key 已保存（仅内存）"),
        "chat.preset_label" => ("Official presets", "官方推荐配置"),
        "chat.preset_pick" => ("Select a provider…", "选择服务商…"),
        "chat.preset_applied" => ("Preset applied", "已应用预设"),
        "chat.preset_open_source" => ("open-source", "开源"),
        "chat.preset_closed_source" => ("closed-source", "闭源"),
        "chat.preset_no_key_needed" => ("no API key needed", "无需 API Key"),
        "chat.preset_key_hint" => ("Get an API key at:", "获取 API Key："),
        "chat.preset_refresh" => ("Refresh online", "在线更新"),
        "chat.preset_refreshing" => ("Fetching official presets…", "正在获取官方推荐配置…"),
        "chat.preset_refreshed" => ("Preset list updated", "预设列表已更新"),
        "chat.preset_refresh_failed" => ("Preset refresh failed", "预设更新失败"),
        "chat.network_consent" => (
            "Allow RusTerm to access the network to fetch the official preset catalog (GitHub). Nothing is uploaded.",
            "允许 RusTerm 访问外网获取官方推荐配置（GitHub）。不会上传任何数据。",
        ),
        "chat.proxy_label" => ("Network proxy", "网络代理"),
        "chat.proxy_system" => ("System (env vars)", "系统（环境变量）"),
        "chat.proxy_off" => ("Direct (no proxy)", "直连（不使用代理）"),
        "chat.proxy_clash" => (
            "Clash auto-detect (7890/7897/7891)",
            "Clash 自动检测（7890/7897/7891）",
        ),
        "chat.proxy_custom" => ("Custom URL", "自定义 URL"),
        "chat.proxy_custom_placeholder" => (
            "http://127.0.0.1:7890 or socks5://127.0.0.1:7891",
            "http://127.0.0.1:7890 或 socks5://127.0.0.1:7891",
        ),
        "chat.proxy_clash_found" => ("Clash proxy detected:", "已检测到 Clash 代理："),
        "chat.proxy_clash_missing" => (
            "No local Clash client detected — falling back to direct connection.",
            "未检测到本地 Clash 客户端——将直连。",
        ),
        "chat.dock_tooltip" => (
