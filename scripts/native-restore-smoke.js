return (async () => {
  ...
  const unlock = await waitFor(
    () => [...document.querySelectorAll("button")]
      .find((button) => button.textContent.trim() === "Unlock" && !button.disabled),
  ...
  if (leftWidth < 260) throw new Error(`resized left dock width was not restored: ${leftWidth}`);
  if (!document.querySelector('[data-rusterm-dock-zone="bottom"]')) {
    throw new Error("bottom dock visibility was not restored");
  }
  if (!document.querySelector("#main")) throw new Error("center workspace missing after restart");
  return "ok";
})();
