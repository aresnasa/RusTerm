return (async () => {
  ...
  const ensureZone = async (zone, toggleTitle) => {
    let element = document.querySelector(`[data-rusterm-dock-zone="${zone}"]`);
    if (!element) {
  ...
  const activateDockTab = async (label) => {
    ...
    await waitFor(
      () => tab.classList.contains("active") || document.querySelector(`[title="Drag to reorder or move ${label}"].active`),
  ...
  await waitFor(() => !createButton.disabled, "Create & Unlock stayed disabled");
  createButton.click();

  await waitFor(() => document.querySelector("#main"), "workspace did not render after unlock");
  ...
  leftHandle.dispatchEvent(new MouseEvent("mousedown", {
    button: 0,
  ...
  const activeHandle = await waitFor(
    () => document.querySelector('[data-rusterm-dock-zone="left"] .dock-resize-handle.active'),
    "left resize did not start",
  );
  const resizeOverlay = activeHandle.previousElementSibling;
  resizeOverlay.dispatchEvent(new MouseEvent("mousemove", {
    button: 0,
    buttons: 1,
  ...
  resizeOverlay.dispatchEvent(new MouseEvent("mouseup", {
    button: 0,
    clientX: handleRect.left + 41,
    clientY: handleRect.top + 30,
    bubbles: true,
  ...
  await waitFor(
    () => document.querySelector('[data-rusterm-dock-zone="left"]').getBoundingClientRect().width > leftBefore + 20,
    "left dock width did not grow",
  );

  const sourceTab = document.querySelector('[title="Drag to reorder or move Remote files"]');
  ...
  sourceTab.dispatchEvent(new MouseEvent("mousedown", {
    button: 0,
  ...
  await waitFor(
    () => document.querySelector('[data-rusterm-dock-zone="right"] [title="Drag to reorder or move Remote files"]'),
    "Remote files did not move to the right dock",
  );
  await waitFor(
    () => window._rusterm_dock_drag_remove == null && document.body.style.userSelect !== "none",
    "dock drag listener or text-selection suppression remained active",
  );
  if (!document.querySelector("#main")) throw new Error("center workspace disappeared after dock move");
  ...
  await sendCommand(mainTerminal, "printf RUSTERM_MAIN_E2E_OK", "RUSTERM_MAIN_E2E_OK");

  await ensureZone("bottom", "Show or hide Send, Shell, and Transfers");
  await activateDockTab("Shell");
  await clickButton("Start local shell");
  const bottomTerminal = await waitFor(
    () => document.querySelector('[data-rusterm-dock-zone="bottom"] [id^="terminal-input-"]'),
    "embedded shell did not open",
  );
  const bottomTerminalId = bottomTerminal.id;
  if (bottomTerminalId === mainTerminal.id) throw new Error("main and bottom shell reused a DOM id");
  await sendCommand(bottomTerminal, "printf RUSTERM_BOTTOM_E2E_OK", "RUSTERM_BOTTOM_E2E_OK");
  ...
  await waitFor(() => buttonByText("Start local shell"), "embedded shell placeholder did not return");
  if (!document.getElementById(mainTerminal.id)) throw new Error("terminating bottom shell closed main terminal");
