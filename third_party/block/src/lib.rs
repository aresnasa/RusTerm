...
// NOTE(2026-07-20): Originally `enum Class { }` (an uninhabited enum), which
// triggered Rust's future-incompat lint `uninhabited_static` (issue #74840).
// Since `_NSConcreteStackBlock` is only ever used as a pointer value
// (`isa: *const Class`) for identity comparison, we replace the empty enum
// with a zero-sized inhabited struct. This silences the warning without
// changing any runtime semantics — `Class` remains opaque and is never
// instantiated by safe code.
//
// `#[repr(C)]` + `[u8; 0]` field keeps the type FFI-safe (zero-sized, with
// defined layout), so the `extern "C"` block below doesn't trip the
// `improper_ctypes` lint that rejects `PhantomData`-only types in FFI.
#[repr(C)]
struct Class {
    _opaque: [u8; 0],
}
